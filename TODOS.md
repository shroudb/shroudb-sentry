...
- [x] **DEBT-5** — bootstrap gate must not reopen after last policy is deleted (one-shot latch). Test: `debt_5_bootstrap_gate_must_not_reopen_after_policy_delete` @ same file.
- [x] **DEBT-6** — `main.rs` must not hardcode Chronicle=None. Test: `debt_6_server_main_must_not_hardcode_chronicle_none` @ same file.
